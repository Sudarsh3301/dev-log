...
![One thread per row: each thread runs all three serial passes — mean loop, variance loop, normalize loop — reading the same row from global memory three separate times](/ln_images/image1.png)
...
<TextWithDiagram
  side="right"
  caption="Block-per-row: 256 threads cooperate on one row instead of one thread owning it."
>
...
<img
  slot="diagram"
  src="/ln_images/image2.png"
  alt="One block per row: blockIdx.x replaces threadIdx.x as the row index — 256 threads now cooperate on a single row rather than one thread owning it serially"
/>
...
![Each thread's partial sum lands in smem[tidx]: 256 values distributed across shared memory, one per thread — the input to the log₂(256)=8 step reduction tree](/ln_images/image4.png)
...
![256 threads = 8 warps of 32: \_\_shfl_down_sync only crosses lanes within a warp — the intra-warp reduction needs no shared memory, but the 8 warp sums still require one smem round-trip to combine](/ln_images/image6.png)
...
![\_\_shfl_down_sync with offsets 16,8,4,2,1: each thread adds the value from the thread offset lanes ahead, reducing 32 lanes to a single warp sum in 5 steps — zero shared memory writes](/ln_images/image8.png)
...
![8 warp sums written to smem[0..7] by each warp's first thread, then the first warp loads all 8, performs one final shuffle reduction, and writes the block sum to smem[0]](/ln_images/image9.png)
...
![Variance reduction mirrors mean reduction: lrvar accumulates v²·sums per thread, warp shuffle reduces within each warp, smem combines the 8 warp results, gvar = smem[0]/n − gmean²](/ln_images/image10.png)
...
![Vectorized load: one float4 instruction fetches 4 consecutive floats in a single 128-bit memory transaction — 256 threads × 4 elements = 1024 elements per iteration vs 256 in scalar coalescing](/ln_images/image11.png)
...
![float4 struct maps v.x, v.y, v.z, v.w to four consecutive memory addresses — one reinterpret_cast loads a 128-bit word so the thread accumulates four elements in a single memory instruction](/ln_images/image12.png)
...
Our vectorized approach is around **0.39 ms faster** than PyTorch — approximately 87% more efficient.
...
![Hand-drawn cat waving goodbye — the worklog's sign-off](/ln_images/byecat.png)
